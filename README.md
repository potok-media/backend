...
В репозитории настроен пайплайн `.github/workflows/docker-publish.yml`.
При каждом пуше или слиянии пул-реквеста в ветки `main` или `master`, GitHub Actions автоматически:
1. Выполняет checkout кода и настраивает Docker Buildx.
2. Проходит авторизацию в безопасном реестре **GitHub Container Registry (ghcr.io)**.
3. Собирает Docker-образы для всех трёх сервисов.
4. Тегирует образы (`latest` и по версиям релизов) и безопасно пушит их в ваш личный репозиторий контейнеров.
